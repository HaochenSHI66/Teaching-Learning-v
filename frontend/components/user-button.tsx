...
export function UserButton() {
  ...
  const handleLogout = useCallback(() => {
    clearAuth();
    // Clear auto-login credentials so we don't auto-login again
    try { window.localStorage.removeItem("auto_login_credentials"); } catch {}
        ...
        >
          ...
          {user.is_admin && (
            <a
              ...
              className="flex w-full items-center gap-2 px-4 py-2.5 text-left text-[13px] text-[var(--tx-3)] transition-colors hover:bg-[var(--sf-3)]"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
